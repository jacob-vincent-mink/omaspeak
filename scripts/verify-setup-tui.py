...
Example: python3 scripts/verify-setup-tui.py --binary ~/.local/bin/omaspeak \
    --model-cache ~/.local/share/omaspeak/models/supertonic-3-openvino
"""
    ...
    with tempfile.TemporaryDirectory(prefix=f"{APP}-setup-e2e-") as scratch:
        ...
        try:
            ...
            if args.customize_review:
                ...
                required_pages = {"Omaspeak runtime", "Omaspeak device", "Omaspeak model", "Omaspeak voice", "Audio device"}
        ...
        if args.customize_review:
            required = ("Select setup", "Omaspeak model", "Omaspeak voice", "Audio device", "Accept setup", "Setup cancelled")
        elif args.customize:
            required = ("Select setup", "Omaspeak runtime", "Omaspeak device", "Setup cancelled.")
        elif args.cancel_before_accept:
