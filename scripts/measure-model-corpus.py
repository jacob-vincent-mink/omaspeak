    ...
    for run in range(args.runs):
        for index, case in enumerate(corpus['cases']):
            for voice in corpus['voices']:
                directory = out / f'run-{run}-case-{index}-voice-{voice}'
                directory.mkdir()
                command = [str(binary), '--config', str(config), 'benchmark', '--text', case['text'],
                ...
                if report['backend']['fallback_used']:
                    raise RuntimeError('fallback invalidates this measurement')
                samples = []
                ...
                results['measurements'].append({'run': run, 'case': case['id'], 'voice': voice,
                ...
                print(f"run {run+1}: {case['id']} / {voice}", flush=True)
