...
Omaspeak's `openvino` feature needs a shared sherpa-onnx build whose ONNX Runtime
...
Supertonic and the tracked sherpa patch that lets Supertonic choose an
accelerated provider per component. The sherpa patch also lets offline models
pass CUDA EP V2 options, including `device_id`, through provider config files.
Those patches are part of the tested runtime contract even when an application
does not use Supertonic itself. OpenVINO 2026.2.1 is pinned
