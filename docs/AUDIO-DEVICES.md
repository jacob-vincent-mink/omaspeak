...
Run `omaspeak setup audio` to select System default or a specific device. The Audio
...
```bash
omaspeak audio-devices --detailed --json
omaspeak setup audio --device 'pipewire:<node.name>' --test
omaspeak setup audio --device 'pipewire:<node.name>' --apply
omaspeak config get audio.device
omaspeak config unset audio.device
```
